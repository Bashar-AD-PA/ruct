...
const CreateAdPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [screensRes, categoriesRes, freqRes] = await Promise.all([
    ...
    const handleCalculateCost = async () => {
        ...
        try {
            ...
            const cost = res.data.data.total_cost;
            setCalculatedCost(cost);
            setForm(p => ({ ...p, total_cost: cost }));
        ...
        <div className="max-w-3xl mx-auto space-y-6 pb-12" dir="rtl">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-white border-[2.5px] border-[var(--color-dark-turquoise)] rounded-2xl p-6 space-y-4 shadow-sm">
                    ...
                    {calculatedCost !== null && (
                        <div className="bg-[var(--color-dark-turquoise)]/10 p-4 rounded-xl border border-[var(--color-dark-turquoise)]/30 flex justify-between items-center">
                            <span className="font-bold text-[var(--color-dark-turquoise)]">التكلفة الإجمالية المقدرة:</span>
                            <span className="font-black text-xl text-[var(--color-gold)]">${calculatedCost.toFixed(2)}</span>
                        </div>
                    )}
                </div>
